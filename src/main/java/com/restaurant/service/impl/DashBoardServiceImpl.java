...
public class DashBoardServiceImpl {
	 ...
	 */
	public List<MostSoldDishesInLastFifteenDays> getMostSoldDishes() {
		List<MostSoldDishesInLastFifteenDays> result = new ArrayList<>();
		...
		dishCount.entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).limit(3)
				.forEachOrdered(e -> result.add(new MostSoldDishesInLastFifteenDays(e.getKey(), e.getValue())));
